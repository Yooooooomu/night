<template>
  <view id="video-detail">
    <view class="video-box">
      <video id="video" :poster="video_poster" :src="video_src"
      controls objectFit="fill" @pause="pause=true">
        <cover-view class="video-cover" @click="pause = false;video.play()" v-if="pause" >
          <cover-image class="icon-pause" src="https://bi09fclg.qcloud.la/img/icon-puase.png" />
        </cover-view>
      </video>
    </view>
    ...
    <view class="introduce">
      ...
      <view class="content">
        <view class="baike">
          <rich-text :nodes="video_baike"></rich-text>
        </view>
        <view class="margin"></view>
        <rich-text :nodes="main_text" class="main"></rich-text>
      </view>
    </view>
    <view class="margin"></view>
...
export default {
  ...
  mounted () {
    console.log(this.$root.$mp.query.vid)
    this.video = wx.createVideoContext('video')
    wx.request({
      url: 'https://bi09fclg.qcloud.la/myapi/index/small_video/index.php',
      data: {
        vid: this.$root.$mp.query.vid
      },
      success: ({data, statusCode, header}) => {
        console.log(data, statusCode, header)
        wx.setNavigationBarTitle({title: data.product_name})
        this.video_poster = data.video_poster
        this.video_src = data.video_src
        this.video_title = data.video_title
        this.video_baike = data.video_baike
